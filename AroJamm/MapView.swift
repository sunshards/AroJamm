...
// Regione avente come centro Napoli
let naplesCoordinates = CLLocationCoordinate2D(latitude: 40.83989, longitude: 14.25197)
let naplesRegion = MKCoordinateRegion(center: naplesCoordinates, span: MKCoordinateSpan(latitudeDelta: 0.06, longitudeDelta: 0.06))


struct MapView: View {
    @StateObject private var userLocation = UserLocation()
    @State private var position: MapCameraPosition = .userLocation(fallback: .region(naplesRegion))
            ...
            ZStack {
                                    ...
                                    VStack {
                                        ...
                                        ZStack {
                                            ...
                                            Button(action: {
                                                ...
                                                Image(nextLocation.imagePath ?? "example") // Inserisco l'emoji sulla mappa
                                                    .resizable()
                ...
                VStack {
                    ...
                    HStack(alignment: .top) {
                        // Pulsante reset
                        MapSingleButton(buttonImageName: "arrow.clockwise") {
                        ...
                        MapDoubleButton(
                            ...
                            bottomButtonImageName: "fork.knife.circle",
                            buttonTapped: { tappedButton in
                                switch tappedButton {
                                case .top:
                                    // Azione pulsante cronologia
                                    showingHistory.toggle()
                                    if showingHistory {
                                        updateMapRegionForHistory() // Chiama la funzione per centrare la mappa sulla cronologia
                                    }
                                case .bottom:
                                    // Azione pulsante food
                                    showingEatingModal = true
                                }
                            })
                    ...
                    HStack(spacing:15) {
                            ...
                            VStack {
                                ...
                                ZStack {
                                    ...
                                    Button(action: {
                                        ...
                                        if let nextLocation = getDirections(currentLocation: userLocation.currentLocation, places: places, selectedCategories: selectedCategories) {
                                            animationOpacity = 1.0
                                            
                                            // Controlla se paths non è vuoto prima di accedere a paths[0]
                                            if !paths.isEmpty {
                                                if let i = places[paths[0].0]?.firstIndex(where: { $0.nome == paths[0].1.nome }) {
                                                    places[paths[0].0]![i].visitato = true
                                                }
                                            }
                                            // Assegnare il valore restituito a nextLocation
                                            self.nextLocation = nextLocation
                                        } else {
                                            resetVisitedFlagsForCategories(selectedCategories, places: &places)
                                        }
                                    },
                                           label: {
                                        HStack(spacing:20) {
                        ...
                        Button(action: {
                            // Azione pulsante go
                            showingMapsAlert = true // Aggiungi questa riga se vuoi mostrare l'alert quando viene premuto il pulsante "GO"
                        }) {
                            VStack {
                                ZStack {
                                    Color.green
                        ...
                        .alert(isPresented: $showingMapsAlert) {
                            Alert(
                                ...
                                primaryButton: .default(Text("Ok")) {
                                    ...
                                    chronology.append(nextLocation)
                                    openMaps()
                ...
                .sheet(isPresented: $showingEatingModal, onDismiss: {
                    ...
                    ScrollView(.horizontal, showsIndicators: false) {
                        HStack(spacing: 20) {
                            Spacer()
                            // Parte del codice che crea le SelectableCard

                            // Quando viene selezionata la pizza
                            SelectableCard(label: "Pizza", imagePath: "pizza", userDefaultsKey: "foods", updateToggle: false) { _ in
                                selectedFoods = ["Pizza"] // Imposta selectedFoods su "Pizza"
                                nextFoodLocation() // Chiama la funzione per trovare la prossima posizione del cibo
                                if !paths.isEmpty{
                                    if let i = food[paths[0].0]?.firstIndex(where: { $0.nome == paths[0].1.nome }) {
                                        food[paths[0].0]![i].visitato = true
                                    }
                                }
                            }

                            // Quando viene selezionato il ristorante
                            SelectableCard(label: "Restaurant", systemImage: "wineglass", userDefaultsKey: "foods", updateToggle: false) { _ in
                                selectedFoods = ["Restaurant"] // Imposta selectedFoods su "Restaurant"
                                nextFoodLocation() // Chiama la funzione per trovare la prossima posizione del cibo
                                if !paths.isEmpty{
                                    if let i = food[paths[0].0]?.firstIndex(where: { $0.nome == paths[0].1.nome }) {
                                        food[paths[0].0]![i].visitato = true
                            ...
                            // Quando viene selezionato il sushi
                            SelectableCard(label: "Sushi", imagePath: "sushi", userDefaultsKey: "foods", updateToggle: false) { _ in
                                selectedFoods = ["Sushi"] // Imposta selectedFoods su "Sushi"
                                nextFoodLocation() // Chiama la funzione per trovare la prossima posizione del cibo
                                if !paths.isEmpty{
                                    if let i = food[paths[0].0]?.firstIndex(where: { $0.nome == paths[0].1.nome }) {
                                        food[paths[0].0]![i].visitato = true
                            ...
                            // Quando vengono selezionati i dolci
                            SelectableCard(label: "Sweets", imagePath: "sweet", userDefaultsKey: "foods", updateToggle: false) { _ in
                                selectedFoods = ["Sweet"] // Imposta selectedFoods su "Sweet"
                                nextFoodLocation() // Chiama la funzione per trovare la prossima posizione del cibo
                                if !paths.isEmpty{
                                    if let i = food[paths[0].0]?.firstIndex(where: { $0.nome == paths[0].1.nome }) {
                                        food[paths[0].0]![i].visitato = true
                ...
                .sheet(isPresented: $showingPostcard, content: {
                    ...
                    Postcard(title: nextLocation.nome, image:nextLocation.imagePath, description: nextLocation.description ?? "", wallpaperName: nextLocation.category ?? "Street Art")
    ...
    func getDirections(currentLocation: CLLocationCoordinate2D, places: [String: [Target]], selectedCategories: Set<String>) -> Target? {
        ...
        for category in selectedCategories {
            if let locations = places[category]?.filter({ !$0.visitato }).sorted(by: { loc1, loc2 in
                let location1 = CLLocation(latitude: loc1.coordinate.latitude, longitude: loc1.coordinate.longitude)
                let location2 = CLLocation(latitude: loc2.coordinate.latitude, longitude: loc2.coordinate.longitude)
                ...
                if let firstLocation = locations.first {
                    paths.append((category, firstLocation))
                }
            }
        }
        
        paths.sort { loc1, loc2 in
            let location1 = CLLocation(latitude: loc1.1.coordinate.latitude, longitude: loc1.1.coordinate.longitude)
            let location2 = CLLocation(latitude: loc2.1.coordinate.latitude, longitude: loc2.1.coordinate.longitude)
            let distance1 = location1.distance(from: CLLocation(latitude: currentLocation.latitude, longitude: currentLocation.longitude))
            let distance2 = location2.distance(from: CLLocation(latitude: currentLocation.latitude, longitude: currentLocation.longitude))
            return distance1 < distance2
        }
        ...
        if !paths.isEmpty {
            // Calcola la rotta dalla posizione corrente alla prossima destinazione
            ...
            Task {
                ...
                if let route = response?.routes.first {
                    // Aggiungi la rotta all'array routes
                    routes.append(route)
                }
            }
            
            // Restituisci la prossima destinazione
            return paths[0].1
        } else {
            // Se non ci sono più destinazioni disponibili, restituisci nil
            return nil
        }
    }
    ...
    func nextFoodLocation() {
        if let savedData = UserDefaults.standard.data(forKey: foodsKey) {
            if let decodedData = try? JSONDecoder().decode(Set<String>.self, from: savedData) {
                selectedCategories = decodedData //qua categoria cibo
            }
        }
        ...
        if let nextLocation = getDirections(currentLocation: userLocation.currentLocation, places: food, selectedCategories: selectedFoods) {
            self.nextLocation = nextLocation
        } else {
            resetVisitedFlagsForCategories(selectedFoods, places: &food)
        }
    }
    ...
    func resetVisitedFlagsForCategories(_ categories: Set<String>, places: inout [String: [Target]]) {
        for category in categories {
            if var categoryPlaces = places[category] {
                for index in categoryPlaces.indices {
                    categoryPlaces[index].visitato = false
                }
                places[category] = categoryPlaces
            }
        }
    }
